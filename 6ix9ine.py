...
async def on_reaction_add(reaction, user):
  if reaction.message.server is None:
      if reaction.emoji == "⚙":
        ...
        embed.add_field(name='', value='``>kick`` - kick a user from the server/guild.\n``>ban`` - ban a user from the server/guild.')
        embed.add_field(name='', value='``>slowclear`` - it will slowy clearing all messages from the channel.\n``>warn`` - warn a user from the server/guild.')
        embed.add_field(name='', value='``>decide`` - this will decide you from what the bot wants.\n``>secretkick`` - this will ban a user without showing to logs.')
        embed.add_field(name='', value='``>secretkick`` - this will kick a user without showing it to logs.\n``>clear`` - this will clear on how many do you like to clear.')
        embed.add_field(name='', value='``>slowmode`` - this will turn on the slow mode.\n``>config_slowmode`` - this will config the slowmode.')
        embed.add_field(name='', value='``>renamerole`` - it will Rename a role.\n``>renameserver`` - it will rename the server/guild name.')
